...
class CopyMangaSource : MangaSource {
    ...
    // ─── Search (JSON API — bypasses Cloudflare 5-second shield on HTML path) ─────────────────
    ...
    override suspend fun getChapterImages(chapterUrl: String): Result<List<String>> =
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                if (urls.isEmpty()) error("拷贝漫画：未找到图片 URL，可能页面结构已变更")
                urls
            }
        }


    // ─── Search JSON Parser (api.mangacopy.com) ───────────────────────────────────────
    //
    // Response shape:
